...
bool Graph<T>::addVertex(const T &in) {
	...
	if (findVertex(in) == NULL){
	    Vertex<T>* ver = new Vertex<T>(in);
	    vertexSet.push_back(ver);
	    return true;
	}
	return false;
}
...
vector<T> Graph<T>::bfs(const T & source) const {
	...
	vertexToProcess.push(start);
    while (!vertexToProcess.empty()){
       Vertex<T>* aux = vertexToProcess.front();
       vertexToProcess.pop();
       res.push_back(aux->info);
       for (auto edge: aux->adj){
           Vertex<T>* aux_adj = edge.dest;
           if (!aux_adj->visited){
               vertexToProcess.push(aux_adj);
               aux_adj->visited = true;
           }
       }
	...
	while (!vertexToProcess.empty()){
        ...
        for (auto edge: aux->adj){
            ...
            if (edge.dest->indegree == 0){
                vertexToProcess.push(edge.dest);
            }
        }
	}
